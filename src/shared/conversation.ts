...
export function serializeForChat(state: ConversationState): ProviderMessage[] {
  const messages: ProviderMessage[] = []
  if (state.context) {
    messages.push({
      role: 'system',
      content: [
        'You are DeltaAI, a helpful assistant in the chat window.',
        'Answer in simple and concise words.'
      ].join('')
    })
    messages.push({
      role: 'user',
      content: `The following context is relevant:\n\n"${state.context}"`
    })
  }
